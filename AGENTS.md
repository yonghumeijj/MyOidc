...
Invite keys are generated by the admin page.

Security behavior:

- Plaintext keys are shown only once immediately after generation.
- The database stores only SHA-256 hashes of keys.
...
That means after the user's browser session expires, they need another unused key to sign in again.
If the desired behavior changes to "use key once, then let user log in repeatedly", add a real
second factor such as password, passkey, or email verification.
...
- Consume keys under the store lock so one key cannot be used twice concurrently.
...
These are acceptable for the current "OpenAI SSO with admin-issued one-time keys" scope.
